    ...
    public  void loadlist()
        {
             ...
             try
            {
                ...
                foreach (var i in list)
                {
                    ...
                    string[] rows = new string[] { i.trkid.ToString(), i.papin.ToString(), name.ToString(), i.gender.ToString(), TimeIn.ToShortTimeString(), i.purpose };
        ...
        private void dg_listQueue_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            //if (e.ColumnIndex != -1 || e.RowIndex != -1)
            //{
            //    string transID = dg_listQueue.SelectedRows[0].Cells[0].Value.ToString();
            //    string purpose = dg_listQueue.SelectedRows[0].Cells[5].Value.ToString();
            //    using (frm_productList f = new frm_productList())
            //    {
            //        f.Tag = transID;
            //        //f.txt_purpose.Text = purpose;
            //        f.ShowDialog();
            //    }
            //}
        }

        private void editPurposeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dg_listQueue.SelectedRows.Count > 0)
            {
                string cn = dg_listQueue.SelectedRows[0].Cells[0].Value.ToString();
                string purpose = dg_listQueue.SelectedRows[0].Cells[5].Value.ToString();

                using (frm_VisitDetail f = new frm_VisitDetail())
                {
                    f.cn = cn;
                    f.txtPorpose.Text = purpose;
                    f.ShowDialog();
                }
            }
        }
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show(this, "You're about to delete this item, Are you sure?", "Confirm", MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
                {                    
                    DataClasses1DataContext db = new DataClasses1DataContext(Properties.Settings.Default.MyConString);
                    string cn = dg_listQueue.SelectedRows[0].Cells[0].Value.ToString();
                    db.ExecuteCommand("DELETE from t_registration WHERE  cn = {0}", cn);
                    loadlist();
                }
